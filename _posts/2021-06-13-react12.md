...
## 서로 다른 타입의 두 엘리먼트는 서로 다른 트리를 만들어 낸다. (Unmount, Mount)
...
## 같은 Element에서 속성 값만 다른 경우(->Update)
...
## 같은 Component에서 같은 prop의 다른 값을 가지는 경우(-> Update)
...
## Compenent의 개수가 변경 되는 경우 Component 동일성 주기
...
> # 전달 받은 props 업데이트에 따른 Rerender 막기
